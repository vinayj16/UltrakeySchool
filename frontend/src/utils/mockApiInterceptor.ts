...
export function setupMockApiInterceptor(axiosInstance: AxiosInstance) {
  ...
  axiosInstance.interceptors.request.use(
    (request) => {
      ...
      // BUT exclude admin account requests and auth endpoints to allow real backend integration
      if (url.includes('/admin/account-requests') || url.includes('/admin/create-credentials') || url.includes('/auth/login')) {
        // Don't intercept - let it go to real backend
        return request
      }
      
      if (url.includes('/schools/') || url.includes('/transactions/') || url.includes('/analytics/') || url.includes('/notifications') || url.includes('/staff') || url.includes('/students') || url.includes('/teachers') || url.includes('/fees') || url.includes('/attendance') || url.includes('/exams') || url.includes('/library') || url.includes('/reports') || url.includes('/dashboard') || url.includes('/users') || url.includes('/departments') || url.includes('/subjects') || url.includes('/classes') || url.includes('/charts/') || url.includes('/metrics/') || url.includes('/statistics/') || url.includes('/membership-plans') || url.includes('/membership-addons') || url.includes('/profile') || url.includes('/pending-requests')) {
        request.adapter = () => Promise.resolve({
          data: {
            success: true,
            data: url.includes('/profile') ? {
              ...
              role: 'superadmin',
              status: 'active'
            } : (url.includes('/pending-requests') ? [
              {
                id: 'req-1',
                fullName: 'Sample School',
  ...
  axiosInstance.interceptors.response.use(
    (response) => {
      ...
      // Exclude admin account requests and create credentials - let real backend response through
      if (url.includes('/admin/account-requests') || url.includes('/admin/create-credentials') || url.includes('/auth/login')) {
        return response
      }
