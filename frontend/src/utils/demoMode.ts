...
export const setDemoUser = (user: any) => {
  ...
  // Set explicit localStorage keys for components that don't use authStore
  if (isBrowser) {
    try {
      window.localStorage.setItem('schoolId', user.schoolId || 'demo-school-id');
      window.localStorage.setItem('institutionId', user.institutionId || 'demo-inst-id');
      window.localStorage.setItem('userId', user.id || 'demo-user-id');
      window.localStorage.setItem('userName', user.name || 'Demo User');
    } catch (e) {
      console.warn('[DemoMode] Failed to set legacy localStorage keys');
    }
  }
