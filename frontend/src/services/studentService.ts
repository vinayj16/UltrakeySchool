...
export const studentService = {
  ...
  async bulkDelete(ids: string[]): Promise<void> {
    try {
      ...
      if (!response.success) {
        throw new Error(response.error || 'Failed to bulk delete students');
      }
    } catch (error) {
  ...
  async getByClass(classId: string): Promise<Student[]> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.error || 'Failed to fetch students by class');
      }
  ...
  async getAttendance(id: string, startDate?: string, endDate?: string): Promise<Record<string, unknown>[]> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.error || 'Failed to fetch student attendance');
      }
  ...
  async getFees(id: string): Promise<Record<string, unknown>[]> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.error || 'Failed to fetch student fees');
      }
  ...
  async exportCSV(filters: StudentFilters = {}): Promise<Blob> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.error || 'Failed to export students as CSV');
      }
  ...
  async exportPDF(filters: StudentFilters = {}): Promise<Blob> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.error || 'Failed to export students as PDF');
      }
