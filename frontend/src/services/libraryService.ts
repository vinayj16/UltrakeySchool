...
export const libraryService = {
  ...
  async issueBook(data: IssueBookInput): Promise<Borrowing> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.error || 'Failed to issue book');
      }
  ...
  async getBorrowings(params?: Record<string, unknown>): Promise<Borrowing[]> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.error || 'Failed to fetch borrowings');
      }
  ...
  async getMyBorrowings(): Promise<Borrowing[]> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.error || 'Failed to fetch my borrowings');
      }
  ...
  async getFines(params?: Record<string, unknown>): Promise<Fine[]> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.error || 'Failed to fetch fines');
      }
  ...
  async payFine(id: string): Promise<Fine> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.error || 'Failed to pay fine');
      }
  ...
  async bulkDeleteBooks(ids: string[]): Promise<void> {
    try {
      ...
      if (!response.success) {
        throw new Error(response.error || 'Failed to bulk delete books');
      }
    } catch (error) {
  ...
  async exportBooksCSV(filters: LibraryFilters = {}): Promise<Blob> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.error || 'Failed to export books as CSV');
      }
  ...
  async exportBooksPDF(filters: LibraryFilters = {}): Promise<Blob> {
    try {
      ...
      if (!response.success || !response.data) {
        throw new Error(response.error || 'Failed to export books as PDF');
      }
