  ...
  persist(
    (set: any) => ({
      ...
      login: async (email: string, password: string) => {
        ...
        try {
          console.log('[AuthStore] Login attempt with email:', email);
          console.log('[AuthStore] Using REAL authentication only - bypass disabled');
          
          // Regular authentication flow for production - NO BYPASS
          const authData = await authService.login({ email, password });
          
          console.log('[AuthStore] Auth data received:', authData);
          console.log('[AuthStore] User from auth data:', authData.user);
          console.log('[AuthStore] User role from auth data:', authData.user?.role);
          ...
          console.log('Login successful:', authData.user?.fullName || authData.user?.email || 'User');
        } catch (error: any) {
      ...
      register: async (userData: any) => {
        ...
        try {
          ...
          console.log('Registration successful:', authData.user?.fullName || authData.user?.email || 'User');
        } catch (error: any) {
          set({
            user: null,
            isAuthenticated: false,
      ...
      initialize: async () => {
        ...
        try {
          ...
          if (authService.isAuthenticated()) {
            // For demo mode, don't hit the network for profile check
            if (isDemo) {
              ...
              console.log('[Auth] Initialized with demo session:', demoUser?.name);
              return;
            }
          ...
          // Session is invalid - clear everything unless we're in demo mode
          const token = authService.getAccessToken();
