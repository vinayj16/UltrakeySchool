...
export const approveRegistration = async (req, res) => {
  try {
    ...
    const shouldSend = !(sendCredentials === false || sendCredentials === 'false');
    if (shouldSend) {
      try {
        const loginUrl = `${process.env.FRONTEND_URL || 'http://localhost:5173'}/login`;
        const emailResult = await sendEmail({
          to: targetEmail,
          subject: 'Your Ultrakey access is ready',
          html: `
            <div style="font-family: 'Inter', sans-serif; max-width: 640px; padding: 20px;">
              <h2 style="color: #0f172a;">Access Granted</h2>
              <p>Hi ${registration.fullName},</p>
              <p>Your Ultrakey portal access has been approved. Use the credentials below to log in:</p>
              <ul>
                <li><strong>Email:</strong> ${targetEmail}</li>
                <li><strong>Password:</strong> ${ownerPassword}</li>
              </ul>
              <p><a href="${loginUrl}" style="color:#2563eb;">Click here to log in</a></p>
              <p>Please change your password after first login to keep your account secure.</p>
              <p>Best,<br />Ultrakey Team</p>
            </div>
          `
        });

        if (!emailResult.success) {
          logger.warn('Credential email could not be delivered', { emailResult });
        }
      } catch (emailError) {
        logger.error('Failed to send credential email', { error: emailError?.message || emailError, registrationId: registration._id });
      }
    }
