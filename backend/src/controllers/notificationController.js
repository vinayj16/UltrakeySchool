...
import smsService from '../services/twilioService.js';
...
const VALID_CHANNELS = ['in-app', 'email', 'sms', 'push'];
...
export const createNotification = async (req, res) => {
  try {
    ...
    if (notificationData.channels && notificationData.channels.includes('sms')) {
...
async function sendSMSNotification(notification) {
  try {
    ...
    const smsPromises = recipientIds.map(async (recipientId) => {
    ...
    logger.info('SMS notifications sent successfully:', { count: recipientIds.length });
  } catch (error) {
