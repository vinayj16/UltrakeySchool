...
import * as bypassAuth from '../config/bypassAuth.js';
...
export const authenticate = async (req, res, next) => {
  try {
    if (!bypassInitialized) {
      bypassAuth.initializeBypassMode();
      bypassInitialized = true;
    }

    if (bypassAuth.bypassModeEnabled) {
      const requestedRole = bypassAuth.resolveBypassRole(req.headers['x-bypass-role'] || req.query.role);
      req.user = bypassAuth.createMockUser(requestedRole);
      return next();
    }
